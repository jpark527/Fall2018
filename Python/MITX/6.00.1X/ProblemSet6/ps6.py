...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for c in string.ascii_lowercase:
            myDict[c] = chr(((ord(c) - 19 + shift) % 26) + 97)
        for c in string.ascii_uppercase:
            myDict[c] = chr(((ord(c) - 13 + shift) % 26) + 65)
        return myDict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        checker = [0] * 26
        for i in range(0, 26):
            newMessage = Message.apply_shift(self, i)
            words = newMessage.split()
            for w in words:
                checker[i] += is_word(self.valid_words, w)
        shift = checker.index(max(checker))
