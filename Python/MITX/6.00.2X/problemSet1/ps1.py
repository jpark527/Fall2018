...
def greedy_cow_transport(cows,limit=10):
    ...
    copy = cows.copy()
    while(count):
        mostWeight = max(copy.values())
        leastWeight = min(copy.values())
        for key,value in copy.items():
            if value == mostWeight:
                if mostWeight + totalWeight <= limit:
                    totalWeight += mostWeight
                    seq.append(key)
                del copy[key]
                break
        if (not copy) or (leastWeight + totalWeight > limit):
            totalWeight = 0
            for key in seq:
                if orig[key]:
                    del orig[key]
            copy = orig.copy()
            totalSeq.append(seq.copy())
            count -= len(seq)
            seq.clear()
    return totalSeq
