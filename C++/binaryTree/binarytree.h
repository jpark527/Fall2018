...
private:
    ...
    node<Data>* findLargestOnLessThanSide(node<Data>* r);
...
void binaryTree<Data>::deleteGreaterThanNode(node<Data>* &child, node<Data>* parent)
{
    ...
    if(child == root)
    {
        if(root->gThan)
        {
            if(root->lThan)
            {
                temp = findSmallestOnGreaterThanSide(root->gThan);
                temp->lThan = root->lThan;
            }
            root = root->gThan;
            delete parent;
        }
    ...
    else
    {
        ...
        {
            parent->gThan = child->lThan;
            delete child;
        }
        else
        {
            delete child;
            parent->gThan = nullptr;
        }
    }
}
...
template<typename Data>
node<Data>* binaryTree<Data>::findLargestOnLessThanSide(node<Data>* r)
{
    if(r)
        while(r->gThan)
        {
...
template<typename Data>
unsigned int binaryTree<Data>::height()
{
    return height(root);
}
 ...
 */
    ...
    {
        if(!unbalancedPoint->lThan || !unbalancedPoint->gThan)
            break;
 ...
 */
        ...
        else
        {
            ...
            unbalancedPoint->lThan = toMove;
            unbalancedPoint->gThan->lThan = nullptr;
            toMove->swap(*unbalancedPoint);
        }
    }
    else if(height(unbalancedPoint->gThan) == 0)
    {
        /*
 ...
 */
        ...
        else
        {
            ...
            unbalancedPoint->gThan = toMove;
            unbalancedPoint->lThan->gThan = nullptr;
            toMove->swap(*unbalancedPoint);
        }
    }
}
