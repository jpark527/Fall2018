...
void MainWindow::convertFile() {
    ...
    for(size_t i=0; i<words.size(); ++i)
        contents += (QString::fromStdString(words[i].data)+" : "+QString::number(words[i].count)+"\n");
...
void MainWindow::alphabetPressed() {
    ...
    parser->getWordsThatStartsWith(button->text()[0].toLatin1(), words);
    contents.clear();
    ui->fileContentsDisplay->clear();
    for(size_t i=words.size()-1; i!=0; --i)
        ui->fileContentsDisplay->append(QString::fromStdString(words[i].data) +
                                        " : " + QString::number(words[i].count) + "\n");
    ui->fileContentsDisplay->repaint();
    words.clear();
